...
public class ModBlocks {
    public static final Block ITEM_GROWER = registerBlock(ItemGrowerBlock::new, AbstractBlock.Settings.create().requiresTool().strength(2.0f).nonOpaque().pistonBehavior(PistonBehavior.BLOCK), new Item.Settings().maxCount(1));

    private static Block registerBlock(Function<AbstractBlock.Settings, Block> blockFactory, AbstractBlock.Settings baseBlockSettings, @Nullable Item.Settings baseItemSettings // can be null
    ) {
        var id = Identifier.of(GrowItems.MOD_ID, "item_grower");
    ...
    public static void registerModBlocks() {
        ...
        ItemGroupEvents.modifyEntriesEvent(ItemGroups.FUNCTIONAL).register(entries -> entries.addAfter(Items.ENCHANTING_TABLE, ITEM_GROWER.asItem()));
    }
}
